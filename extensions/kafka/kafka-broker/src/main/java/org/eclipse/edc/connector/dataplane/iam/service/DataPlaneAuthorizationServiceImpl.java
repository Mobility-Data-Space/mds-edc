...
public class DataPlaneAuthorizationServiceImpl implements DataPlaneAuthorizationService {
    ...
    public Result<DataAddress> createEndpointDataReference(DataFlow dataFlow) {
        if (dataFlow.getTransferType().destinationType().equals(KafkaBrokerDataAddressSchema.KAFKA_TYPE)) {
            // custom kafka EDR can be created here
            return Result.success(DataAddress.Builder.newInstance().type("cuco").property("endpoint", "cuasdadsagfds").build());
        }
