...
### Next Steps
The immediate priority is to inherit the data plane codebase into the `mds-edc` repository. 

Beyond that, it is worth auditing which transfer types are actually in use and trimming the implementation accordingly. 
For example, if DFRS relies exclusively on Kafka-pull and HTTP-push, those two modes can be implemented cleanly and 
directly — avoiding the complexity of the original "any source to any destination" generic model.

These boundaries should be well distinguishable in the code through module definitions, guaranteeing a good 
