...
The DFRS Observer feature enables a participant connector to automatically establish a contractual channel to a designated
...
The asset `@id` (`dfrs-event-sink` in the example) is the **dataset ID** that must be communicated to participant connectors so they can identify the correct offer in the catalog.
...
The participant connector requires five configuration properties to enable the DFRS observer feature. If any property is missing, the feature is disabled at startup with a warning log.

| Property | Description | Example |
|---|---|---|
| `edc.mds.dfrs.observer.id` | Participant ID of the observer connector | `did:web:observer.example.com` |
| `edc.mds.dfrs.observer.url` | DSP protocol URL of the observer connector | `https://observer.example.com/api/dsp` |
| `edc.mds.dfrs.observer.dataset.id` | Dataset ID of the observer's event-sink offer | `dfrs-event-sink` |
| `edc.mds.dfrs.observer.profile` | DSP communication profile | `dataspace-protocol-http:2025-1` |
| `edc.mds.dfrs.observer.transfer.profile` | Transfer profile for the observer channel | `HttpData-PULL` |

### Example: `config.properties`

```properties
edc.mds.dfrs.observer.id=did:web:observer.example.com
edc.mds.dfrs.observer.url=https://observer.example.com/api/dsp
edc.mds.dfrs.observer.dataset.id=dfrs-event-sink
...
```bash
EDC_MDS_DFRS_OBSERVER_ID=did:web:observer.example.com
EDC_MDS_DFRS_OBSERVER_URL=https://observer.example.com/api/dsp
EDC_MDS_DFRS_OBSERVER_DATASET_ID=dfrs-event-sink
EDC_MDS_DFRS_OBSERVER_PROFILE=dataspace-protocol-http:2025-1
EDC_MDS_DFRS_OBSERVER_TRANSFER_PROFILE=HttpData-PULL
```
